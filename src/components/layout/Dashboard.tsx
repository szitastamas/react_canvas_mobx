...
const Dashboard = () => {
  const { allBlocks, deepestPosition, rightestPosition, addBlock } = useContext(
    rootStore
  ).nodeStore;
    ...
    >
      <div className="node-block-container">
        {allBlocks.map((block: INodeBlock) => (
          <NodeBlock key={block.id} block={block} recalculateDashboardSize={recalculateDashboardSize} />
        ))}
      </div>
